        ...
        if 'state' in feat_dict:
            ...
            if 'P' in feat_dict:
                if g['p2s'].number_of_edges() > 0:
                    ...
                    g['p2s'].dstdata.update({'Attn_dst_p2s': Attn_dst_p2s}) # DGLError: Expect number of features to match number of edges. Got 7 (nfeats) and 3 (nedges) instead.
            ...
            if 'A' in feat_dict:
                ...
                if g['a2s'].number_of_edges() > 0:
                    ...
                    g['a2s'].dstdata.update({'Attn_dst_a2s': Attn_dst_a2s}) # DGLError: Expect number of features to match number of edges. Got 7 (nfeats) and 3 (nedges) instead.
...
class MultiHeteroGATLayerReal(nn.Module):
    ...
    def forward(self, g, feat_dict, last_layer=False):
        # print('feat_dict in MultiHeteroGATLayerReal', feat_dict)
