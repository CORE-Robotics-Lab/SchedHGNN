...
class MAPPO_GAT_Policy(R_MAPPOPolicy):
    ...
    def build_hetgraph_2class(self, inputs):
        ...
        if self.args.with_state:
            if self.args.with_two_state:
                num_nodes_dict = {'P': self.args.num_P, 'A': self.args.num_A, 'state': self.args.num_P + self.args.num_A}  # 3}
            else:
                num_nodes_dict = {'P': self.args.num_P, 'A': self.args.num_A, 'state': self.args.num_A}
        else:
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_obs_actor, rnn_states_critic, masks, available_actions=None,
                    deterministic=False):
        """
        ...
        actions, action_log_probs, rnn_states_actor, rnn_obs_actor, state_features, all_action_log_probs = self.actor(obs,
                                                                                                state,
                                                                                                rnn_states_actor,
                                                                                                rnn_obs_actor,
                                                                                                masks,
        ...
        return values, actions, action_log_probs, rnn_states_actor, rnn_obs_actor, rnn_states_critic, all_action_log_probs
    
    def get_values(self, cent_obs, obs, rnn_states_actor, rnn_obs_actor, rnn_states_critic, masks, available_actions=None):
        values, _, _, _, _, _, _ = self.get_actions(cent_obs, obs, rnn_states_actor, rnn_obs_actor, rnn_states_critic, masks, available_actions)
        return values
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_obs_actor,  rnn_states_critic, action, masks,
        ...
        action_log_probs, dist_entropy, state_features, all_action_log_probs = self.actor.evaluate_actions(obs,
                                                                     state,
                                                                     rnn_states_actor,
                                                                     rnn_obs_actor,
                                                                     action,
        ...
        return values, action_log_probs, dist_entropy, all_action_log_probs

    def act(self, obs, rnn_states_actor, rnn_obs_actor, masks, available_actions=None, deterministic=False):
        """
        ...
        :param deterministic: (bool) whether the action should be mode of distribution or should be sampled.
        """
        # TODO: remove cent obs
        g = self.build_hetgraph(obs)
        
        actions, _, rnn_states_actor, rnn_obs_actor, _, _ = self.actor(obs, None, rnn_states_actor, rnn_obs_actor, masks, available_actions, deterministic, graph=g)
