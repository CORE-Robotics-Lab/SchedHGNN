...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.action_have_vision = ('Marine' in args.env_name)
        self.heterogeneous_env = ('Marine' in args.env_name)
        obs_shape = get_shape_from_obs_space(obs_space)
        share_obs_shape = get_shape_from_obs_space(cent_obs_space)
        print('Buffer, heterogeneous_env', self.heterogeneous_env)
        print("Buffer, action_have_vision", self.action_have_vision)
        print('obs_shape, share_obs_shape', obs_shape, share_obs_shape)
        ...
        if hasattr(args, 'tensor_obs') and args.tensor_obs:

            self.obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *obs_shape), dtype=object)
        else:
        ...
        if args.use_LSTM:
            if hasattr(args, 'tensor_obs') and args.tensor_obs:
                # hidden size of state hidden vectors: 128
                # hidden size of obs hidden vectors: 16
                if not hasattr(args, 'stat_hidden_dim') and not hasattr(args, 'obs_hidden_dim'):
                    self.args.stat_hidden_dim = 128
                    self.args.obs_hidden_dim = 16
                print('buffer, rnn_states.shape: ', self.args.stat_hidden_dim)
                print('buffer, rnn_obs.shape: ', self.args.obs_hidden_dim)
                self.rnn_states = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents * 2,
                                            self.recurrent_N, self.args.stat_hidden_dim), dtype=np.float32)
                ...
                if self.action_have_vision:
                    ...
                    self.rnn_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents * 2,
                                                self.recurrent_N, self.args.obs_hidden_dim), dtype=np.float32)
                else:
                    self.rnn_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, args.num_P * 2,
                                             self.recurrent_N, self.args.obs_hidden_dim), dtype=np.float32)
            else:
                self.rnn_states = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents * 2,
                                            self.recurrent_N, self.hidden_size), dtype=np.float32)
                if self.heterogeneous_env:
                    if self.action_have_vision:
                        # action agents also have vision
                        self.rnn_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents * 2,
        ...
        self.rnn_states_critic = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, self.recurrent_N,
                                    self.hidden_size),dtype=np.float32)

        self.value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.returns = np.zeros_like(self.value_preds)
        # print("buffer, L118, act_space.__class__.__name__", act_space.__class__.__name__) # Discrete
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_obs_actor, rnn_states_critic, actions, action_log_probs,
        ...
        :param rnn_obs_actor: (np.ndarray) RNN states for actor network. RNN states associated with the observation input.
        ...
        self.rnn_obs[self.step+1] = rnn_obs_actor.copy()
    ...
    def insert1(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        """
        Insert data into the buffer.
        :param share_obs: (argparse.Namespace) arguments containing relevant model, policy, and env information.
    ...
    def sample_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        share_obs_batch = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[3:])



        obs_batch = self.obs[:-1].reshape(-1, *self.obs.shape[2:])
    ...
    def sample_generator1(self, advantages, num_mini_batch=None, mini_batch_size=None):
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3]
        batch_size = n_rollout_threads * episode_length * num_agents
        if mini_batch_size is None:
            assert batch_size >= num_mini_batch, (
        ...
        if hasattr(self.args, 'tensor_obs') and self.args.tensor_obs:
            ...
            if self.action_have_vision:
                # action agents also have vision
                rnn_obs_batch = self.rnn_obs[:-1].reshape(-1, 2 * num_agents, self.args.obs_hidden_dim)
            else:
                rnn_obs_batch = self.rnn_obs[:-1].reshape(-1, 2 * self.args.num_P, self.args.obs_hidden_dim)
        else:
            rnn_states_batch = self.rnn_states[:-1].reshape(-1, 2 * num_agents, self.args.hidden_size)
            rnn_obs_batch = self.rnn_obs[:-1].reshape(-1, 2 * num_agents, 16)


        rnn_states_critic_batch = self.rnn_states_critic[:-1].reshape(-1, *self.rnn_states_critic.shape[3:])
        actions_batch = self.actions.reshape(-1, self.actions.shape[-1])
        if self.available_actions is not None:
            available_actions_batch = self.available_actions[:-1].reshape(1, -1, self.available_actions.shape[-1])
        else:
            available_actions_batch = None
        value_preds_batch = self.value_preds[:-1].reshape(-1, 1)
        return_batch = self.returns[:-1].reshape(-1, 1)
        ...
        yield share_obs_batch, obs_batch, rnn_states_batch, rnn_obs_batch, rnn_states_critic_batch, actions_batch, \
              value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, \
              adv_targ, available_actions_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        # rnn_states = self.rnn_states.squeeze(1)
        # rnn_states_critic = self.rnn_states_critic.squeeze(1)
